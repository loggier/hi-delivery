...
export async function POST(request: Request) {
  try {
    ...
    // We only validate the fields for the first step
    ...
    if (insertError) {
      ...
      if (insertError.code === '23505') { // unique_violation for email or phone
    ...
    // For now, to simplify and debug, we shape the rider data like a User object for the auth store.
    const userForSession = {
        id: createdRider.id, // Use the RIDER ID for the session for now
